...
/// Main window: live MJPEG feed, connection/gamepad status, IP settings sheet, and a debug
/// panel mirroring the "CMD / RES" readout in `app/page.tsx`.
struct ContentView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showSettings) {
            SettingsView(client: client, stream: stream, isPresented: $showSettings)
        }
        ...
        .onAppear {
            ...
            if let url = client.settings.streamURL {
    ...
    private var header: some View {
        HStack {
            Label(
                client.settings.ip.isEmpty ? "No camera configured" : "\(client.settings.ip):\(client.settings.port)",
            ...
            Text(controlLoop.autoFocus ? "AF" : "MF")
                .font(.caption2).bold()
                .foregroundStyle(controlLoop.autoFocus ? .green : .secondary)
